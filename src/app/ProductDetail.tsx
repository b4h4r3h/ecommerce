...
const ProductDetail: React.FC<{}> = () => {
    ...
    return (
      <div className="flex gap-4 items-center">
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          <span key={i} className="block w-full bg-gray-middle h-4 mt-4 rounded-lg"></span>
              ))}
              </div>
  ...
  return (
    ...
    items={[
      ...
      {
        title: <a href={`/products/category/${productDetailData?.category}`}>{productDetailData?.category}</a>,
      },
      {
    ...
    <div className="flex gap-6 items-center mt-6">
      ...
      <div>
        ...
        <Tag color="#DDF6C9" className="!border-secondary-low">
          <a className="!text-secondary" href={`/products/category/${productDetailData?.category}`}>
        ...
        <Button className="h-11 w-96 font-bold text-base" type="primary">ADD TO CART - {productDetailData?.price}$</Button>
      </div>
    </div>
    </>
  );
